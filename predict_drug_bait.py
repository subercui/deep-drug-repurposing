...
def graph_embedding(config, msi, gcn=False):
    ...
    baits = config['covid']["baits"]

    # covid_emb = np.array(node_embs[node_names.index('NodeCovid')])
    covid_embs = np.stack(
        [np.array(node_embs[node_names.index(bait)]) for bait in baits], axis=1)
    ...
    proximities = np.matmul(drug_embs, covid_embs)
    proximities = proximities.max(1)
...
def main(config):
    ...
    if not os.path.exists(covid_to_protein):
        ...
        covid_bait_prey_pairs = make_SARSCOV2_PPI_baits(
            gordon_viral_protein)  # 332

        # has 306
        covid_edge_list = [
            (bait, protein) for (bait, protein) in covid_bait_prey_pairs if protein in proteins['node_1_name'].values]

        proteinname2node = dict(
            set(list(zip(proteins['node_1_name'], proteins['node_1']))))
        # make covid data fram
        node_1 = [bait for bait, _ in covid_edge_list]
        node_1_type = ['indication'] * len(covid_edge_list)
        node_1_name = [bait for bait, _ in covid_edge_list]
        node_2 = [proteinname2node[name] for _, name in covid_edge_list]
        node_2_type = ['protein'] * len(covid_edge_list)
        node_2_name = [name for _, name in covid_edge_list]
    ...
    output_drugs(drugs_name_ranked, drugs_ranked,
                 proximities_ranked, topk, save_drug_candidates_dir, msi, config)
    return msi


def output_drugs(drugs_name_ranked, drugs_ranked, proximities_ranked,
                 topk, save_drug_candidates_dir, msi, config):
    # df = pd.DataFrame({'drug name': drugs_name_ranked[:topk]})
    # df.to_csv(save_drug_candidates_dir, sep='\t', na_rep='NA', index=False)
    baits = config['covid']["baits"]
    msi.node2name['NodeCovid'] = 'Covid-19'
    for bait in baits:
        msi.graph.add_edge(bait, 'NodeCovid')
        msi.graph.add_edge('NodeCovid', bait)
