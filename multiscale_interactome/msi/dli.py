...
class DLI():
    """
    The adjacency matrix has complete connectivity information of
    drug, disease, protein and pathway nodes.
    """

    def __init__(
        self,
        nodes=[DRUG, INDICATION, PROTEIN, FUNCTIONAL_PATHWAY],
        edges=[DRUG_PROTEIN, INDICATION_PROTEIN, PROTEIN_PROTEIN,
               PROTEIN_FUNCTIONAL_PATHWAY, FUNCTIONAL_PATHWAY_FUNCTIONAL_PATHWAY],
            drug2protein_file_path="data/drug_to_protein.tsv",
            drug2protein_directed=False,
            indication2protein_file_path="data/indication_to_protein.tsv",
            indication2protein_directed=False,
            protein2protein_file_path="data/protein_to_protein.tsv",
            protein2protein_directed=False,
            protein2functional_pathway_file_path="data/protein_to_functional_pathway.tsv",
            protein2functional_pathway_directed=False,
            functional_pathway2functional_pathway_file_path="data/functional_pathway_to_functional_pathway.tsv",
            functional_pathway2functional_pathway_directed=True
    ):
        # Parameters
        self.nodes = nodes
        self.edges = edges
    ...
    def add_pathway_in(self, protein_pathway):
        """adds connections between pathway and indication/disease"""
        protein_pathway_graph = protein_pathway.graph
        assert not nx.is_directed(protein_pathway_graph)
        pathway_nodes = [
            node for node in protein_pathway_graph.nodes if protein_pathway.node2type[node] == FUNCTIONAL_PATHWAY]
        edge_list = []
        for node in self.graph.nodes:
            # for disease node
            if (self.graph.nodes[node]['type'] == INDICATION) or (self.graph.nodes[node]['type'] == DRUG):
                source_adjs = set(self.graph.adj[node])
                for pathway in pathway_nodes:
                    path_adjs = set(protein_pathway_graph.adj[pathway])
                    if len(source_adjs.intersection(path_adjs)) > 0:  # share_connected_protein
                        edge_list.append((node, pathway))
        # add edges
        for node, pathway in edge_list:
            self.graph.add_edge(node, pathway)
            self.graph.nodes[pathway]["type"] = FUNCTIONAL_PATHWAY
    ...
    def merge_one_to_one_dicts(self, dict_list):
        ...
        for dict_ in dict_list:
            for k, v in dict_.items():
                if k in out_dict:
                    assert((out_dict[k] == v) or (
                        pd.isnull(out_dict[k]) and pd.isnull(v)))
                else:
                    out_dict[k] = v
        return out_dict
    ...
    def load_graph(self):
        """
        register nodes and edges, form node2name mapping, 
        register protein2patheay connection, and construct 
        ...
        if (DRUG in self.nodes) and (DRUG_PROTEIN in self.edges):
            self.components["drug_to_protein"] = DrugToProtein(
                self.drug2protein_directed, self.drug2protein_file_path)
            self.add_edges(
                self.components["drug_to_protein"].edge_list, DRUG, PROTEIN)

        if (INDICATION in self.nodes) and (INDICATION_PROTEIN in self.edges):
            self.components["indication_to_protein"] = IndicationToProtein(
                self.indication2protein_directed, self.indication2protein_file_path)
            self.add_edges(
                self.components["indication_to_protein"].edge_list, INDICATION, PROTEIN)

        if (PROTEIN in self.nodes) and (PROTEIN_PROTEIN in self.edges):
            self.components["protein_to_protein"] = ProteinToProtein(
                self.protein2protein_directed, self.protein2protein_file_path)
            self.add_edges(
                self.components["protein_to_protein"].edge_list, PROTEIN, PROTEIN)

        if (FUNCTIONAL_PATHWAY in self.nodes) and (PROTEIN_FUNCTIONAL_PATHWAY in self.edges):
            self.components["protein_to_functional_pathway"] = ProteinToFunctionalPathway(
                self.protein2functional_pathway_directed, self.protein2functional_pathway_file_path)

            # add connections between drug/disease and functional pathways
            self.add_pathway_in(
                self.components["protein_to_functional_pathway"])

        # if (FUNCTIONAL_PATHWAY in self.nodes) and (FUNCTIONAL_PATHWAY_FUNCTIONAL_PATHWAY in self.edges):
        #     self.components["functional_pathway_to_functional_pathway"] = FunctionalPathwayToFunctionalPathway(
        #         self.functional_pathway2functional_pathway_directed, self.functional_pathway2functional_pathway_file_path)
        #     self.add_edges(
        #         self.components["functional_pathway_to_functional_pathway"].edge_list, FUNCTIONAL_PATHWAY, FUNCTIONAL_PATHWAY)

        # add_pathway_pathway edges?
    ...
    def load_drug_or_indication2proteins(self):
        drug_or_indication2proteins = dict.fromkeys(
            self.drugs_in_graph + self.indications_in_graph)
        for drug in self.drugs_in_graph:
            assert(
                not(nx.is_directed(self.components["drug_to_protein"].graph)))
            drug_or_indication2proteins[drug] = set(
                self.components["drug_to_protein"].graph.neighbors(drug))
        for indication in self.indications_in_graph:
            assert(
                not(nx.is_directed(self.components["indication_to_protein"].graph)))
            drug_or_indication2proteins[indication] = set(
                self.components["indication_to_protein"].graph.neighbors(indication))
    ...
    def create_class_specific_adjacency_dictionary(self):
        ...
        for node in self.graph.nodes():
            ...
            if node_type == FUNCTIONAL_PATHWAY:
                up_neighbors = list(
                    self.components["functional_pathway_to_functional_pathway"].graph.successors(node))
                down_neighbors = list(
                    self.components["functional_pathway_to_functional_pathway"].graph.predecessors(node))
            ...
            for successor in successors:
                ...
                if (node_type == FUNCTIONAL_PATHWAY) and (successor_type == FUNCTIONAL_PATHWAY):
                    if successor in up_neighbors:
                        self.add_to_cs_adj_dict(
                            node, UP_FUNCTIONAL_PATHWAY, successor)
                    elif successor in down_neighbors:
                        self.add_to_cs_adj_dict(
                            node, DOWN_FUNCTIONAL_PATHWAY, successor)
                    else:
                        assert(False)
                else:
        ...
        for from_node, adj_dict in self.cs_adj_dict.items():
            for node_type, to_nodes in adj_dict.items():
                ...
                for to_node in to_nodes:
                    self.graph[from_node][to_node][WEIGHT] = weights[node_type] / \
                        float(num_typed_nodes)
